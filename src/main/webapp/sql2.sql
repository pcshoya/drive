...
SELECT 
    TO_CHAR(TO_DATE(D.DRV_DATE, 'YYYYMMDD'), 'YYYY"년"MM"월"DD"일"') AS 주행일자,
    D.CAR_NO AS 차량번호,
    C.CAR_MODEL AS 모델,
    TO_CHAR(D.DRV_START, '999,999') AS 출발KM,
    TO_CHAR(D.DRV_END, '999,999') AS 도착KM,
    T.DEPT_NAME AS 부서명,
    TO_CHAR(D.DRV_MONEY, 'L999,999') AS 주유금액
FROM TBL_DRIVE D
JOIN TBL_CAR C
    ON D.CAR_NO = C.CAR_NO
JOIN TBL_DEPT T
